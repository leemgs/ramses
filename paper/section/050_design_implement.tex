...
\caption{Operating map for the two pressure ratios. Here $\alpha=D/C_f$ denotes capacity pressure and $\beta$ is the ratio of non-overlapped transfer time to compute, memory, and synchronization time. The dashed lines are operational classification thresholds used by the controller, not feasibility, determinism, or stability guarantees. Under the evaluated workloads RAMSES operates in the coordination-dominated region ($0.6<\beta<0.9$), whereas the baselines run at higher mean transfer pressure and approach the transfer-limited boundary ($\beta\to1$) under burst load; the corresponding measured behavior is reported in Section~\ref{sec:results}.}
