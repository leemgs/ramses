...
The \textbf{GPU Booster with swap support} targets this bottleneck by selecting inactive tensors and scheduling block-aligned asynchronous transfers. It is implemented as an \texttt{LD\_PRELOAD} interception layer over the CUDA runtime and caching allocator: allocation, free, and host--device copy calls are intercepted so that tensor placement and movement are managed without modifying the model or the serving framework. Intercepted entry points, buffer-registration details, and exact driver/filesystem versions are documented with the review artifact.

Conventional offloading typically pays a noticeable transfer cost, and OS-level swapping is unaware of GPU memory semantics, which limits how well it can do. The GPU Booster instead uses execution-graph reuse distance to rank eviction candidates, dedicated asynchronous CUDA streams to overlap transfer with compute, and 4~MB block alignment matched to the NVMe erase-block granularity; the block-size choice is examined in the sensitivity study of Section~\ref{sec:results}. On hosts that expose GPUDirect Storage, tensors move over a direct VRAM--NVMe DMA path; otherwise the layer falls back to a pinned-host-buffer staged path with the same asynchronous overlap. Partial reload fetches only the portion of a swapped tensor required by the next kernel, and output equivalence against the unoptimized baseline is checked bitwise for FP32 and within the precision tolerance for FP16.

The goal is to serve models whose state exceeds VRAM capacity while keeping exposed transfer latency low. Algorithm~\ref{alg:gbooster_en} summarizes the control flow.
\begin{algorithm}[t]
...
\caption{Operating map for the two pressure ratios. Here $\alpha=D/C_f$ denotes capacity pressure and $\beta$ is the ratio of non-overlapped transfer time to compute, memory, and synchronization time. The dashed lines are operational classification thresholds used by the controller, not feasibility, determinism, or stability guarantees. Under the evaluated workloads RAMSES operates in the coordination-dominated region ($0.6<\beta<0.9$), whereas baselines drift toward the transfer-pressured boundary ($\beta\to1$); the corresponding measured behavior is reported in Section~\ref{sec:results}.}
...
worst-case bound.  Fig.~\ref{fig:phase_diagram} summarizes the resulting
...
coordination-region behavior, so the system drifts toward the transfer-pressured
