...
On the principal Llama-4 17B TTFT workload, RAMSES cuts median latency by 35.8\% (871.6~ms to 563.0~ms) and raises token throughput by 61\% relative to PyTorch (Table~\ref{tab:latency_percentiles}; all records at batch~1).
...
\textbf{Results.}
Measured across the whole node, RAMSES drops energy per token by 46.7\% and
energy per inference by 46.7\% relative to PyTorch, and the energy--delay product
by 69.6\%---latency and energy fall together rather than trading off. Because
