...
failures, and GPU energy, and where does its overhead cease to pay? We retain
...
Energy is instrumented on the same monotonic clock as the request and GPU
telemetry.  The primary energy results are GPU-only NVML measurements (detailed
in the energy--latency subsection below); NVML GPU energy is never substituted
for CPU, DRAM, or NVMe consumption.  Each run records the instrument model, sampling rate, clock
alignment, idle-subtraction procedure, calibration uncertainty, and dropped-
sample count.  The measurement schema and analysis programs are included under
\texttt{../code/data/}.  Synchronized whole-node CPU/DRAM/NVMe/GPU energy
integration requires rack-level metering beyond the GPU-only scope of this
study and is identified as future work in Section~\ref{sec:discussion}.
...
Each configuration was run five times; the per-run observations and Student-$t$ confidence intervals are provided in the artifact. Generalization beyond these five architectures is not claimed and remains to be established on further model families.
...
When synchronized whole-node CPU/DRAM/NVMe/GPU energy is available, Table~\ref{tab:energy} reports the absolute per-request and per-token energy, EDP, and throughput integrated across all tiers.
...
the same trace and seed (Table~\ref{tab:policy_off}).
...
reuse window, locating the operating points used elsewhere in this section.
...
while latency, VRAM, and energy improve. Table~\ref{tab:industrial_accuracy}
