...
The \textbf{GPU Booster with swap support} design targets this bottleneck by selecting inactive tensors and scheduling block-aligned asynchronous transfers. The repository currently contains only an MBALL proof-of-concept whose GPU allocation is mocked and whose disk swap is explicitly an emulation; it does not contain a production GPUDirect Storage path. Consequently, direct VRAM--NVMe DMA, 4~MB optimality, and partial-reload claims require a versioned runtime plus driver/filesystem registration evidence before they can be treated as implemented features.
...
The intended mechanism uses execution-graph reuse distance to rank candidates, asynchronous queues to overlap transfer and compute, and configurable block alignment. Algorithm~\ref{alg:gbooster_en} is therefore design pseudocode rather than a claim that an unmodified framework can provide tensor-level partial reload through \texttt{LD_PRELOAD}. A complete artifact must identify intercepted APIs or framework patches, CUDA/GDS and filesystem versions, buffer registration, metadata locking, cancellation and failure recovery, and output equivalence.

The design target is to serve models whose state exceeds VRAM capacity while reducing exposed transfer latency. Algorithm~\ref{alg:gbooster_en} documents the proposed control flow; it is not experimental evidence of direct DMA or partial reload. Those mechanisms and their output equivalence must be validated by the missing runtime artifact.
\begin{algorithm}[t]
...
\caption{Schematic operating map for the measured pressure ratios. Here $\alpha=D/C_f$ denotes capacity pressure and $\beta$ is the ratio of non-overlapped transfer time to compute, memory, and synchronization time. The dashed lines are operational classification thresholds, not feasibility, determinism, or stability guarantees; measured operating points and prediction residuals are required before this schematic can be treated as a validated predictor.}
...
worst-case bound.  Fig.~\ref{fig:phase_diagram} is therefore only a schematic guide. Measured operating points and prediction residuals are not present in the repository and remain required before resubmission.
...
and metadata occupies $O(n)$.  The required policy-off ablation would retain all three data-plane modules, fix prefetch depth, and disable state switching, thereby separating the controller from its mechanisms. Results for that experiment are not yet available.  The reported 1.68~ms is mean controller
...
\end{tabular}
\vspace{0.3ex}
{\footnotesize Values are author-reported mean $\pm$ standard deviation over five runs. Raw observations are required to recompute confidence intervals and statistical tests before submission.}
\end{table}
