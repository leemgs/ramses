...
used as a laboratory server (not claimed to represent a deployed factory edge):
...
and vLLM~0.5.3. Llama-4 experiments use a documented local compatibility adapter rather than claiming native vLLM~0.5.3 support; the adapter and invocation are part of the artifact. The principal workload is Llama-4 (Maverick-17B-128E-Instruct)~\cite{meta2025llama4}. The machine has
...
These checks establish similarity only to the stated reference summaries; they do not establish field representativeness. The generator, seed, and extraction procedure are included in the review artifact.
